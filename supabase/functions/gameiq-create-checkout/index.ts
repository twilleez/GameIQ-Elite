...
Deno.serve(async(req:Request)=>{
  ...
  if(!stripeKey||!proPrice||!teamPrice)return json({error:'Billing is not configured'},503);
  ...
  if(!plan)return json({error:'Plan must be pro or team'},400);
  const price=plan==='team'?teamPrice:proPrice;
  ...
  form.set('metadata[profile_id]',claims.sub);form.set('metadata[plan]',plan);
  form.set('subscription_data[metadata][profile_id]',claims.sub);form.set('subscription_data[metadata][plan]',plan);form.set('allow_promotion_codes','true');
